...
//		mbId: this.$session.get('memberInfo')
    ...
    methods:{ 
		...
		itemJoinLists:function(){
			...
			axios.get(url, {params : searchParam})
			.then(
				(res)=>{
					console.log(res);
			...
			axios.post(url, 
			{
			...
			
			 {
				...
				(res)=>{
					...
					// this.$router.push("/");
					// const router = new VueRouter({path : "/OrderDone", component : OrderDone, name : "OrderDone"});
					// router.beforeEach((to, from, next)=>{
					// 	console.log("dd");
					// 	eventBus.$emit("cartPay", formData , list );
					// 	next();
					// });
					this.$router.push({
						path : "/OrderDone", component : OrderDone, name : "OrderDone", query : {formData : formData, list : list}
					});
