  ...
  <div>
	...
	<table width="90%" cellspacing="0" cellpadding="0">
		...
		<tr>
			...
			<td height="50" class="td_default"><input class="input_default"
				type="text" id="ordNm" name="ordNm" v-model="ordNm" size="20" maxlength="20"
		...
		<tr rowspan="2">
			...
			<td height="50" class="td_default">
				<!-- 다음주소 시작--> <input type="text" v-model="ordPost" name="ordPost" id="sample4_postcode"
				...
				<input type="text" v-model="ordAddr1" name="ordAddr1" id="sample4_roadAddress"
				placeholder="도로명주소" style="width: 330px;height: 35px;"> <input type="text" v-model="ordAddr2" name="ordAddr2"
		...
		<tr>
			<td height="50" class="td_default" style="text-indent: 10px;">휴대전화</td>
			<td height="50" class="td_default"><input class="input_default"
				type="text" id="phone1" v-model="phone1" size="5" maxlength="5" value="" style="width: 100px;height: 35px;">-<input
				class="input_default" type="text" id="phone2" v-model="phone2" size="5" maxlength="5"
				value="" style="width: 100px;height: 35px;">-<input class="input_default" type="text"
				id="phone3" v-model="phone3" size="5" maxlength="5" value="" style="width: 100px;height: 35px;"></td>
	...
	<table width="90%" cellspacing="0" cellpadding="0">
		...
		<tr>
			<td width="125" height="50" class="td_default"><input
				type="radio" name="payMeth" v-model="payMeth" value="신용카드" checked>신용카드 <input
				type="radio" name="payMeth" v-model="payMeth" value="계좌이체">계좌이체 <input
				type="radio" name="payMeth" v-model="payMeth" value="무통장 입금">무통장 입금</td>
...
//		mbId: this.$session.get('memberInfo')
	...
	created:function(){
		...
		eventBus.$on("itemConfirm", this.receive);
    ...
    methods:{ 
		...
		execDaumPostcode(){ //@click을 사용할 때 함수는 이렇게 작성해야 한다.
            ...
            oncomplete: (data) => { //function이 아니라 => 로 바꿔야한다.
               ...
               this.ordPost = data.zonecode;
               this.ordAddr1 = roadAddr;
		...
		receive:function(itCd,amount,optCd){
			console.log("receive 호출됨");
			this.itCd = itCd;
			this.amount = amount;
			this.optCd = optCd;
			console.log("receive>>>>>"+this.itCd+"\t"+this.amount+"\t"+this.optCd);
		},


		itemJoinLists:function(){
			var itemList = this.list;
			var CartLists = this.cartCdList;
			var itCd = this.itCd;
			var amount = this.amount;
			var optCd = this.optCd;

			axios.post("http://localhost:8093/app/check/orderConfirm/"+`${itCd}/${amount}/optCd/${optCd}`)
			.then(
				(res)=>{
					console.log(res);
					res.data.body.map(function(ele,idx){
						console.log("ele>>>",ele);
						itemList.push(ele);
						// console.log("ele.cartCd>>>>",ele.cartCd);
//						 this.cartCdList.push(ele.cartCd);

					});
			
					console.log(itemList.length);
					console.log(itemList[0].mbId);
					itemList.map(function(ele,idx){
						console.log("ele.cartCd>>>>"+ele.cartCd);
						CartLists.push(ele.cartCd);
					})
					console.log("cartCd 배열>>>>>>>>>"+this.cartCdList);
        ...
        pay: function(){ //유효성 검사
			...
			var ordNm = document.getElementById("ordNm").value;
...
//				Event.preventDefault;
			...
			if(ordNm.length==0){
				alert("필수 입력 항목");
				document.getElementById("ordNm").focus();
				return false;
			}
			...
			console.log("payMeth>>>"+this.payMeth);
			

			const searchParam = {
				cartCd: this.cartCdList.join(",")
			}

			

			var url = "http://localhost:8093/app/check/orderDone"
			
			var phone1=this.phone1 ;
			var phone2=this.phone2 ;
			var phone3=this.phone3 ;
			var list = this.list;
			const formData = new FormData()
			formData.append("ordCd",this.ordCd); 
			formData.append("dlvyCd",this.dlvyCd); 
			formData.append("ordNm",this.ordNm); 
			formData.append("ordPost",this.ordPost); 
			formData.append("ordAddr1",this.ordAddr1); 
			formData.append("ordAddr2",this.ordAddr2); 
			formData.append("ordTel",this.ordTel); 
			formData.append("payMeth",this.payMeth); 
			formData.append("payTm",this.payTm); 
			...
			axios.post(url, 
			{
				...
				dlvyCd:this.dlvyCd,
				ordNm:this.ordNm,
				ordPost:this.ordPost,
				ordAddr1:this.ordAddr1,
				ordAddr2:this.ordAddr2,
				ordTel: phone1+phone2+phone3,
			...
			
			 {
				...
				(res)=>{
					console.log("성공");
					eventBus.$emit("itemPay", formData , list );
				}
			).catch(
				(error)=>{
    ...
    watch:{
        isChecked(val){
            if(val){
				this.ordNm = document.getElementById("mname").value;
				this.ordPost = document.getElementById("mpost").value;
				this.ordAddr1 = document.getElementById("maddress1").value;
				this.ordAddr2 = document.getElementById("maddress2").value;
				this.phone1=document.getElementById("mphone1").value;
				this.phone2=document.getElementById("mphone2").value;
				this.phone3=document.getElementById("mphone3").value; 
            }else{
				this.ordNm ="";
				this.ordPost ="";
				this.ordAddr1 = "";
